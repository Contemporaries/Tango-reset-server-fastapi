...
async def get_polling_list_and_info(dev_name: str):
    return service_polling.get_polling_list_and_info(dev_name=dev_name)
...
async def start_polling(dev_name: str, prop_name: str, period: int):
    return service_polling.start_polling(
        dev_name=dev_name, prop_name=prop_name, period=period
    )


@polling_router.post(path="/polling/stop", description="Stop polling a device.")
async def stop_polling(dev_name: str, prop_name: str):
    return service_polling.stop_polling(dev_name=dev_name, prop_name=prop_name)
...
async def set_ring_depth(dev_name: str, depth: int):
    return service_polling.set_ring_depth(dev_name=dev_name, depth=depth)
...
async def start_cmd_polling(dev_name: str, cmd_name: str, period: int):
    return service_polling.start_cmd_polling(
        dev_name=dev_name, cmd_name=cmd_name, period=period
    )
...
async def stop_cmd_polling(dev_name: str, cmd_name: str):
    return service_polling.stop_cmd_polling(dev_name=dev_name, cmd_name=cmd_name)
